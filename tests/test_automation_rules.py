...
def test_rule_crud(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert r["conditions"] == {"project_id": 1} and r["action_params"] == {
            "user_id": 2
        }
...
def test_matches_verb_kind_conditions(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        any_verb = automation.create_rule(
            conn, name="any", trigger_verb="*", action_type="comment", created_by=1
        )
        created = automation.create_rule(
            conn, name="c", trigger_verb="created", action_type="comment", created_by=1
        )
        other = automation.create_rule(
            conn,
            name="o",
        ...
        assert (
            automation._matches(conn, any_verb, {**ev, "target_kind": "page"}) is False
        )

        # Conditions are an AND of issue-field equalities resolved at match time.
        proj = automation.create_rule(
            conn,
            name="p",
            trigger_verb="created",
            action_type="comment",
        ...
        proj_wrong = automation.create_rule(
            conn,
            name="pw",
            trigger_verb="created",
            action_type="comment",
            conditions={"project_id": pid + 999},
            created_by=1,
        )
...
def test_process_pending_fires_matches_and_advances_cursor(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        in_p = client.post(
            "/issues", json={"title": "a", "project_id": pid}, headers=H1
        ).json()["id"]
...
def test_same_status_project_move_does_not_fire_status_rule(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert automation.process_pending(conn, executor=lambda *_args: None) == 0
        ...
        assert (
            automation.process_pending(
                conn,
                executor=lambda _conn, _rule, event: calls.append(event["verb"]),
            )
            == 0
        )
        assert calls == []
...
def test_disabled_rule_and_loop_guard(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        conn = db.connect(db_file)
        rule = automation.create_rule(
            conn, name="r", trigger_verb="created", action_type="comment", created_by=1
        )
        calls: list[int] = []
        ...
        assert (
            automation.process_pending(conn, executor=ex, skip_actor_id=1) == 0
            and calls == []
        )
