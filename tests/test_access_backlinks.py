...
def test_page_backlinks_hide_a_private_issue_source(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        sid = client.post("/spaces", json={"key": "ENG", "name": "Eng"}, headers=H_CREATOR).json()["id"]
        pg = client.post(f"/spaces/{sid}/pages", json={"title": "Target"}, headers=H_CREATOR).json()["id"]
        pp = client.post("/projects", json={"name": "Secret", "key": "SEC"}, headers=H_CREATOR).json()["id"]
        hi = client.post("/issues", json={"title": "Hidden ref", "project_id": pp, "body": f"see [[page:{pg}]]"}, headers=H_CREATOR).json()["id"]
        pi = client.post("/issues", json={"title": "Public ref", "body": f"see [[page:{pg}]]"}, headers=H_CREATOR).json()["id"]
        conn = db.connect(db_file)
        conn.execute("UPDATE projects SET visibility = 'private' WHERE id = ?", (pp,))
        conn.commit()

        def sources(headers=None):
            return {(b["kind"], b["id"]) for b in client.get(f"/pages/{pg}/backlinks", headers=headers or {}).json()}
...
def test_issue_backlinks_hide_a_private_page_source(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        target = client.post("/issues", json={"title": "Target issue"}, headers=H_CREATOR).json()["id"]
        ...
        hp = client.post(f"/spaces/{ps}/pages", json={"title": "Hidden doc", "body": f"see [[issue:{target}]]"}, headers=H_CREATOR).json()["id"]
        pp_ = client.post(f"/spaces/{os_}/pages", json={"title": "Public doc", "body": f"see [[issue:{target}]]"}, headers=H_CREATOR).json()["id"]
        conn = db.connect(db_file)
        conn.execute("UPDATE spaces SET visibility = 'private' WHERE id = ?", (ps,))
        conn.commit()

        def sources(headers=None):
            return {(b["kind"], b["id"]) for b in client.get(f"/issues/{target}/backlinks", headers=headers or {}).json()}
