...
def test_project_visibility_toggle_authz(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert (
            client.put(
                url, json={"visibility": "private"}, headers=H_OUTSIDER
            ).status_code
            == 403
        )
        ...
        assert (
            client.put(
                url, json={"visibility": "secret"}, headers=H_CREATOR
            ).status_code
            == 422
        )
...
def test_going_private_hides_project_and_auto_adds_creator(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert any(
            p["id"] == pid for p in client.get("/projects", headers=H_OUTSIDER).json()
        )
        ...
        assert any(
            p["id"] == pid for p in client.get("/projects", headers=H_CREATOR).json()
        )
        ...
        assert (
            client.get(f"/projects/{pid}/statuses", headers=H_OUTSIDER).status_code
            == 404
        )
        assert (
            client.get(f"/projects/{pid}/statuses", headers=H_CREATOR).status_code
            == 200
        )
...
def test_project_member_lifecycle_and_roster_gate(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert (
            client.post(
                members_url, json={"user_id": 999}, headers=H_CREATOR
            ).status_code
            == 422
        )
        ...
        assert (
            client.post(
                members_url, json={"user_id": 3}, headers=H_OUTSIDER
            ).status_code
            == 404
        )
        ...
        def made_private(actor):
            return [
                ...
                for e in activity.list_activity(
                    conn, target_kind="project", target_id=pid, actor=actor
                )
...
def test_space_visibility_toggle_and_member_lifecycle(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert (
            client.put(
                vis_url, json={"visibility": "private"}, headers=H_OUTSIDER
            ).status_code
            == 403
        )
        assert (
            client.put(
                vis_url, json={"visibility": "nope"}, headers=H_CREATOR
            ).status_code
            == 422
        )
        ...
        assert not any(
            s["id"] == sid for s in client.get("/spaces", headers=H_OUTSIDER).json()
        )
        assert client.get(f"/spaces/{sid}", headers=H_OUTSIDER).status_code == 404
        # Creator auto-recorded as a member.
        assert [
            m["user_id"]
            for m in client.get(f"/spaces/{sid}/members", headers=H_CREATOR).json()
        ] == [2]
        ...
        assert client.get(members_url, headers=H_OUTSIDER).status_code == 404
        assert (
            client.post(members_url, json={"user_id": 3}, headers=H_CREATOR).status_code
            == 201
        )
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        def made_private(actor):
            return [
                e
                for e in activity.list_activity(
                    conn, target_kind="space", target_id=sid, actor=actor
                )
