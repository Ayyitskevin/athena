...
|---|---|---|
| Issue core | Create; title/body/status/priority; assignee, project, and sprint placement; blocked-close policy enforcement/override; parent hierarchy; archive/restore; labels; contributors/delegation; typed dependency link/unlink; REST/browser comment create/edit/delete | The comment commands keep authorization — issue visibility and the author-ownership rule — at the transport boundary and trust the resolved ids the route passes; the command owns the write and its audit, not the gate. A future caller reaching the command directly bypasses both checks until the gate moves inside |
...
| Rooms | Create, archive, and append-only event posting share `room_commands`; each re-resolves the live actor, bearer credential, scope, room audience, and controlled reference under `BEGIN IMMEDIATE`, then commits the activity fact, immutable room metadata, and FTS projection together. Project/issue/member commands create and re-scope invariant linked rooms in their own transactions; REST, browser, and MCP use those owners. | No known split write. Main and brief rooms are intentionally non-archivable; work-item and agent archive is irreversible, while messages remain inert and delivery-ineligible. |
...
| Personal state | None — by design, and bounded by the rules in the next section: saved filters, watches, and notification read-marks are owner-scoped in their mutation SQL (single-statement writes, or one immediate transaction where a read feeds the write), record no audit events, and have exactly one data-module writer each | The category is an exception, not a lane: anything that fails those rules is not personal state and needs a command owner like every other write |
