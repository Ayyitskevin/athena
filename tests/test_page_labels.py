...
def test_attach_detach_and_labels_on_pageout(tmp_path):
    with TestClient(create_app(tmp_path / "api.db")) as client:
        ...
        attached = client.post(f"/pages/{pid}/labels", json={"label_id": lid}, headers=H1)
        ...
        assert [la["id"] for la in client.get(f"/pages/{pid}").json()["labels"]] == [lid]
        ...
        assert len(client.post(f"/pages/{pid}/labels", json={"label_id": lid}, headers=H1).json()["labels"]) == 1
        ...
        assert client.delete(f"/pages/{pid}/labels/{lid}", headers=H1).status_code == 404
...
def test_attach_guards(tmp_path):
    with TestClient(create_app(tmp_path / "guards.db")) as client:
        ...
        assert client.post(f"/pages/{pid}/labels", json={"label_id": 9999}, headers=H1).status_code == 422
        assert client.post("/pages/9999/labels", json={"label_id": 1}, headers=H1).status_code == 404
...
def test_web_add_by_name_and_remove(tmp_path):
    with TestClient(create_app(tmp_path / "web.db")) as client:
        ...
        assert client.post(f"/mentor/pages/{pid}/labels", data={"name": "frontend"}, follow_redirects=False).status_code == 303
        assert "frontend" in client.get(f"/mentor/pages/{pid}").text

        lid = client.get(f"/pages/{pid}").json()["labels"][0]["id"]
        assert client.post(f"/mentor/pages/{pid}/labels/{lid}/delete", follow_redirects=False).status_code == 303
        assert client.get(f"/pages/{pid}").json()["labels"] == []
        # Removing from a missing page is a 404 (symmetric with add).
        assert client.post(f"/mentor/pages/9999/labels/{lid}/delete", follow_redirects=False).status_code == 404
...
def test_web_label_requires_login(tmp_path):
    with TestClient(create_app(tmp_path / "anon.db")) as client:
        ...
        assert anon.post(f"/mentor/pages/{pid}/labels", data={"name": "x"}, follow_redirects=False).status_code in (401, 403)
        assert client.get(f"/pages/{pid}").json()["labels"] == []
