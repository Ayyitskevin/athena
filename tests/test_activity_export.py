...
def test_csv_shape_and_headers(tmp_path):
    with TestClient(create_app(tmp_path / "shape.db")) as client:
        ...
        assert (
            ...
            == 'attachment; filename="athena-activity.csv"'
        )
        rows = _rows(r.text)
        ...
        assert (
            rows[0]["detail"] == "open → in_progress" and rows[0]["actor_name"] == "Ann"
        )
...
def test_filters_match_the_feed(tmp_path):
    with TestClient(create_app(tmp_path / "filter.db")) as client:
        ...
        only_created = _rows(
            client.get("/activity/export.csv?verb=created", headers=H1).text
        )
...
def test_target_id_requires_kind(tmp_path):
    with TestClient(create_app(tmp_path / "tgt.db")) as client:
        _seed(client)
        assert (
            client.get("/activity/export.csv?target_id=1", headers=H1).status_code
            == 422
        )
        # With the kind, it's a valid one-target export.
        ok = client.get(
            "/activity/export.csv?target_id=1&target_kind=issue", headers=H1
        )
...
def test_limit_caps_and_pages(tmp_path):
    with TestClient(create_app(tmp_path / "limit.db")) as client:
        ...
        assert (
            client.get("/activity/export.csv?limit=99999", headers=H1).status_code
            == 422
        )
        ...
        nxt = _rows(
            client.get(
                f"/activity/export.csv?limit=1&before_id={first[0]['id']}", headers=H1
            ).text
        )
