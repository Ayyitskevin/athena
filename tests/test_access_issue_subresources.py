...
def test_subresource_404s_for_outsider_on_private_issue(tmp_path, sub):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        client.post(
            f"/issues/{iid}/comments", json={"body": "secret note"}, headers=H_CREATOR
        )
...
def test_children_list_hides_a_private_child_under_a_public_parent(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        v = client.post(
            "/issues", json={"title": "Public parent"}, headers=H_CREATOR
        ).json()["id"]
        ...
        c = client.post(
            "/issues",
            json={"title": "Acquire FooCorp", "project_id": pp},
            headers=H_CREATOR,
        ).json()["id"]
