...
def test_web_create_issue_into_private_project_blocked(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert (
            conn.execute(
                "SELECT COUNT(*) AS n FROM issues WHERE project_id=?", (pp,)
            ).fetchone()["n"]
            == 0
        )
...
def test_web_move_issue_into_private_project_blocked(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        pp, conn = _private_project(client, db_file)
        biid = client.post(
            "/issues", json={"title": "mine"}, headers=H_OUTSIDER
        ).json()["id"]  # backlog, outsider's
        _login(client, "o@e.com")
        r = client.post(
            f"/aegis/issues/{biid}/project",
            data={"project_id": str(pp)},
            follow_redirects=False,
        )
        assert r.status_code == 400
        assert (
            conn.execute(
                "SELECT project_id FROM issues WHERE id=?", (biid,)
            ).fetchone()["project_id"]
            is None
        )
...
def test_web_link_and_parent_to_hidden_issue_blocked(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        src = client.post("/issues", json={"title": "Mine"}, headers=H_OUTSIDER).json()[
            "id"
        ]  # backlog, outsider's
        _login(client, "o@e.com")
        # Link to the hidden issue → same 400 as a missing target (no oracle, no write).
        r = client.post(
            f"/aegis/issues/{src}/links",
            data={"relation": "blocks", "target_ref": str(hidden)},
            follow_redirects=False,
        )
        assert r.status_code == 400
        # Nest under the hidden issue → same 400.
        r = client.post(
            f"/aegis/issues/{src}/parent",
            data={"parent_ref": str(hidden)},
            follow_redirects=False,
        )
        assert r.status_code == 400
        # Neither write landed.
        assert (
            conn.execute("SELECT COUNT(*) AS n FROM issue_links").fetchone()["n"] == 0
        )
        assert (
            conn.execute("SELECT parent_id FROM issues WHERE id=?", (src,)).fetchone()[
                "parent_id"
            ]
            is None
        )
...
def test_web_board_move_blocked_when_issue_hidden(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        iid = client.post(
            "/issues", json={"title": "card", "project_id": pp}, headers=H_OUTSIDER
        ).json()["id"]
        ...
        conn = db.connect(db_file)
        assert (
            conn.execute("SELECT status FROM issues WHERE id=?", (iid,)).fetchone()[
                "status"
            ]
            == before
        )
...
def test_rest_set_parent_to_hidden_issue_blocked(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        r = client.put(
            f"/issues/{child}/parent", json={"parent_id": hidden}, headers=H_OUTSIDER
        )
