...
def _page(client, sid):
    return client.post(f"/spaces/{sid}/pages", json={"title": "Doc"}, headers=H1).json()["id"]
...
def test_page_renders_thread_and_web_post(tmp_path):
    with TestClient(create_app(tmp_path / "web.db")) as client:
        ...
        assert page.status_code == 200 and "Comments" in page.text and "Add a comment" in page.text

        posted = client.post(
            f"/mentor/pages/{pid}/comments", data={"body": "first post"}, follow_redirects=False
        )
...
def test_blank_comment_rejected(tmp_path):
    with TestClient(create_app(tmp_path / "blank.db")) as client:
        ...
        pid = _page(client, _space(client))
        assert client.post(
            f"/mentor/pages/{pid}/comments", data={"body": "  "}, follow_redirects=False
        ).status_code == 400
...
def test_delete_is_author_only(tmp_path):
    with TestClient(create_app(tmp_path / "gate.db")) as client:
        ...
        pid = _page(client, _space(client))
        client.post(f"/mentor/pages/{pid}/comments", data={"body": "mine"}, follow_redirects=False)
        ...
        ok = client.post(f"/mentor/pages/{pid}/comments/{cid}/delete", follow_redirects=False)
...
def test_logged_out_reader_sees_signin_prompt(tmp_path):
    with TestClient(create_app(tmp_path / "anon.db")) as client:
        _login(client)
        pid = _page(client, _space(client))
        client.post(f"/mentor/pages/{pid}/comments", data={"body": "hello"}, follow_redirects=False)
        ...
        assert "Add a comment" not in page.text and "Sign in</a> to comment" in page.text
