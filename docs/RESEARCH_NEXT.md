...
| 4 | **API safety for agent loops** | V1 safety shipped | Durable single-flight idempotency, stable official-client keys, token request limits, issue bulk updates, and atomic issue ETag/If-Match guards exist. | Extend conditional writes to pages and remaining issue sub-resources only when their writes move behind atomic commands |
