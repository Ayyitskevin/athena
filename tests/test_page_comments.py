...
def test_add_list_oldest_first_with_author(tmp_path):
    with TestClient(create_app(tmp_path / "add.db")) as client:
        ...
        first = client.post(
            f"/pages/{pid}/comments", json={"body": "first"}, headers=H1
        )
...
def test_blank_body_is_422(tmp_path):
    with TestClient(create_app(tmp_path / "blank.db")) as client:
        ...
        assert (
            client.post(
                f"/pages/{pid}/comments", json={"body": "   "}, headers=H1
            ).status_code
            == 422
        )
...
def test_unknown_page_is_404(tmp_path):
    with TestClient(create_app(tmp_path / "missing.db")) as client:
        ...
        assert (
            client.post(
                "/pages/9999/comments", json={"body": "x"}, headers=H1
            ).status_code
            == 404
        )
...
def test_author_only_edit_and_delete(tmp_path):
    with TestClient(create_app(tmp_path / "author.db")) as client:
        ...
        pid = _page(client, _space(client))
        cid = client.post(
            f"/pages/{pid}/comments", json={"body": "mine"}, headers=H1
        ).json()["id"]
        ...
        assert (
            client.delete(f"/pages/{pid}/comments/{cid}", headers=H2).status_code == 403
        )
        # The author can edit, then delete.
        edited = client.patch(
            f"/pages/{pid}/comments/{cid}", json={"body": "mine v2"}, headers=H1
        )
        assert edited.status_code == 200 and edited.json()["body"] == "mine v2"
        assert (
            client.delete(f"/pages/{pid}/comments/{cid}", headers=H1).status_code == 204
        )
...
def test_comment_must_belong_to_the_named_page(tmp_path):
    with TestClient(create_app(tmp_path / "wrongpage.db")) as client:
        ...
        cid = client.post(
            f"/pages/{p1}/comments", json={"body": "on one"}, headers=H1
        ).json()["id"]
        ...
        assert (
            client.delete(f"/pages/{p2}/comments/{cid}", headers=H1).status_code == 404
        )
...
def test_comment_and_delete_land_on_the_activity_trail(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        pid = _page(client, _space(client))
        cid = client.post(
            f"/pages/{pid}/comments", json={"body": "hi"}, headers=H1
        ).json()["id"]
