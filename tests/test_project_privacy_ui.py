...
def test_toggle_visibility_and_manage_members_via_web(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        r = client.post(f"/aegis/projects/{pid}/visibility", data={"visibility": "private"}, follow_redirects=False)
        assert r.status_code == 303
        # The project is now private and the creator was auto-added to the roster.
        conn = db.connect(db_file)
        assert conn.execute("SELECT visibility FROM projects WHERE id=?", (pid,)).fetchone()["visibility"] == "private"
        ...
        client.post(f"/aegis/projects/{pid}/members", data={"user_id": "3"}, follow_redirects=False)
...
def test_access_page_authz(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert client.post(f"/aegis/projects/{pid}/visibility", data={"visibility": "public"}, follow_redirects=False).status_code == 404
