...
def test_backlinks_existence_oracle_closed(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert client.get(f"/issues/{hidden}/backlinks", headers=H_OUTSIDER).status_code == 404
        assert client.get(f"/issues/{hidden}/backlinks").status_code == 404
        assert client.get("/issues/999999/backlinks", headers=H_OUTSIDER).status_code == 404
        assert client.get(f"/issues/{hidden}/backlinks", headers=H_CREATOR).status_code == 200
...
def test_privacy_manage_404s_for_outsider_on_private(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        _bootstrap(client)
        pp = client.post("/projects", json={"name": "Secret", "key": "SEC"}, headers=H_CREATOR).json()["id"]
        sp = client.post("/spaces", json={"key": "SEC", "name": "Secret"}, headers=H_CREATOR).json()["id"]
        conn = db.connect(db_file)
        conn.execute("UPDATE projects SET visibility='private' WHERE id=?", (pp,))
        ...
        assert client.put(f"/projects/{pp}/visibility", json={"visibility": "public"}, headers=H_OUTSIDER).status_code == 404
        assert client.post(f"/projects/{pp}/members", json={"user_id": 3}, headers=H_OUTSIDER).status_code == 404
        assert client.delete(f"/projects/{pp}/members/2", headers=H_OUTSIDER).status_code == 404
        assert client.put(f"/spaces/{sp}/visibility", json={"visibility": "public"}, headers=H_OUTSIDER).status_code == 404
        assert client.post(f"/spaces/{sp}/members", json={"user_id": 3}, headers=H_OUTSIDER).status_code == 404
        assert client.delete(f"/spaces/{sp}/members/2", headers=H_OUTSIDER).status_code == 404
        ...
        assert client.put(f"/projects/{pp}/visibility", json={"visibility": "private"}, headers=H_CREATOR).status_code == 200
        from athena.core import access
        access.add_project_member(conn, pp, 3, added_by=2)
        assert client.put(f"/projects/{pp}/visibility", json={"visibility": "private"}, headers=H_OUTSIDER).status_code == 403
...
def test_status_filter_dropdown_hides_private_custom_status(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        client.post(f"/projects/{pp}/statuses", json={"name": "secretphase", "category": "doing"}, headers=H_CREATOR)
        client.post("/issues", json={"title": "h", "project_id": pp, "status": "secretphase"}, headers=H_CREATOR)
        conn = db.connect(db_file)
        conn.execute("UPDATE projects SET visibility='private' WHERE id=?", (pp,))
        conn.commit()
...
def test_sprint_filter_dropdown_hides_private_sprint(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        _bootstrap(client)
        pp = client.post("/projects", json={"name": "Secret", "key": "SEC"}, headers=H_CREATOR).json()["id"]
        client.post(f"/projects/{pp}/sprints", json={"name": "secretsprintname"}, headers=H_CREATOR)
        conn = db.connect(db_file)
        conn.execute("UPDATE projects SET visibility='private' WHERE id=?", (pp,))
        conn.commit()
