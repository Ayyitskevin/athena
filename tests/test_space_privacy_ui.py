...
def test_toggle_visibility_and_manage_members_via_web(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        r = client.post(
            f"/mentor/spaces/{sid}/visibility",
        ...
        assert (
            conn.execute("SELECT visibility FROM spaces WHERE id=?", (sid,)).fetchone()[
                "visibility"
            ]
            == "private"
        )
...
def test_hidden_space_edit_form_and_delete_do_not_leak(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        _bootstrap(client)
        sid = client.post(
            "/spaces", json={"key": "HUSH", "name": "Skunkworks"}, headers=H_CREATOR
        ).json()["id"]
        conn = db.connect(db_file)
