...
| 4 | **API safety for agent loops** | Partial V1 shipped | Durable single-flight idempotency, token request limits, and issue bulk updates exist; stable client key propagation and update-race protection remain. | Propagate stable keys through official clients, then add ETag/If-Match |
