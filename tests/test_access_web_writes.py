...
def test_issue_web_writes_require_visibility(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert (
            client.post(
                f"/aegis/issues/{iid}/watch", follow_redirects=False
            ).status_code
            == 404
        )
...
def test_project_edit_form_404s_for_outsider_no_leak(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert (
            client.post(
                f"/aegis/projects/{pid}/edit",
                data={"name": "x", "key": "SEC", "description": ""},
                follow_redirects=False,
            ).status_code
            == 404
        )
...
def test_page_and_space_web_writes_require_visibility(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert (
            client.post(
                f"/mentor/pages/{pgid}/watch", follow_redirects=False
            ).status_code
            == 404
        )
        assert (
            client.post(
                f"/mentor/spaces/{sid}/edit",
                data={"key": "SEC", "name": "x", "description": ""},
                follow_redirects=False,
            ).status_code
            == 404
        )
        assert (
            client.post(
                f"/mentor/spaces/{sid}/pages",
                data={"title": "new", "body": "", "parent_id": ""},
                follow_redirects=False,
            ).status_code
            == 404
        )
