...
def test_lifecycle_start_then_complete_with_guards(tmp_path):
    with _client(tmp_path) as client:
        ...
        assert client.post(f"/sprints/{s['id']}/complete", headers=H1).status_code == 409
...
def test_management_is_project_creator_only(tmp_path):
    with _client(tmp_path) as client:
        ...
        assert client.post(f"/projects/{pid}/sprints", json={"name": "X"}, headers=H2).status_code == 403
...
def test_update_and_validation(tmp_path):
    with _client(tmp_path) as client:
        ...
        assert client.patch(f"/sprints/{s['id']}", json={}, headers=H1).status_code == 422
        assert client.patch(f"/sprints/{s['id']}", json={"name": "  "}, headers=H1).status_code == 422
        assert client.post(f"/projects/{pid}/sprints", json={"name": ""}, headers=H1).status_code == 422
...
def test_assign_issue_to_sprint_and_backlog(tmp_path):
    with _client(tmp_path) as client:
        ...
        in_sprint = client.put(f"/issues/{iss['id']}/sprint", json={"sprint_id": s["id"]}, headers=H1)
        ...
        back = client.put(f"/issues/{iss['id']}/sprint", json={"sprint_id": None}, headers=H1)
...
def test_assign_rejects_unknown_and_cross_project_sprint(tmp_path):
    with _client(tmp_path) as client:
        ...
        iss = client.post("/issues", json={"title": "x", "project_id": p1}, headers=H1).json()

        # Unknown sprint, and a sprint from another project, are both 422.
        assert client.put(f"/issues/{iss['id']}/sprint", json={"sprint_id": 9999}, headers=H1).status_code == 422
        assert client.put(f"/issues/{iss['id']}/sprint", json={"sprint_id": s2["id"]}, headers=H1).status_code == 422
...
def test_delete_guarded_by_membership(tmp_path):
    with _client(tmp_path) as client:
        ...
        s = _sprint(client, pid)
        iss = client.post("/issues", json={"title": "x", "project_id": pid}, headers=H1).json()
        client.put(f"/issues/{iss['id']}/sprint", json={"sprint_id": s["id"]}, headers=H1)
