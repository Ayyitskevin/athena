...
def test_issue_list_project_dropdown_hides_private_project(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        _bootstrap(client)
        client.post(
            "/projects", json={"name": "Secretproj", "key": "SEC"}, headers=H_CREATOR
        )
        client.post(
            "/projects", json={"name": "Openproj", "key": "OPN"}, headers=H_CREATOR
        )
...
def test_label_usage_counts_respect_visibility(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        hi = client.post(
            "/issues", json={"title": "h", "project_id": pp}, headers=H_CREATOR
        ).json()["id"]
        ...
        os_ = client.post(
            "/spaces", json={"key": "OSP", "name": "O"}, headers=H_CREATOR
        ).json()["id"]
        hp = client.post(
            f"/spaces/{ps}/pages", json={"title": "hp"}, headers=H_CREATOR
        ).json()["id"]
        op = client.post(
            f"/spaces/{os_}/pages", json={"title": "op"}, headers=H_CREATOR
        ).json()["id"]
        for iid in (hi, pi):
            client.post(
        ...
        for pid in (hp, op):
            client.post(
                f"/pages/{pid}/labels", json={"label_id": lid}, headers=H_CREATOR
            )
        conn = db.connect(db_file)
