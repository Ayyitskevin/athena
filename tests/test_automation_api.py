...
def test_create_rejects_malformed_specs(tmp_path):
    with _client(tmp_path) as client:
        ...
        assert post(action_type="assign", action_params={"user_id": "two"}).status_code == 422
...
def test_pause_resume_and_delete(tmp_path):
    with _client(tmp_path) as client:
        _two_users(client)
        rid = client.post("/automation/rules", json=_valid_rule(), headers=H1).json()["id"]

        paused = client.patch(f"/automation/rules/{rid}", json={"enabled": False}, headers=H1)
        assert paused.status_code == 200 and paused.json()["enabled"] is False
        resumed = client.patch(f"/automation/rules/{rid}", json={"enabled": True}, headers=H1)
...
def test_unknown_rule_is_404(tmp_path):
    with _client(tmp_path) as client:
        ...
        assert client.patch("/automation/rules/999", json={"enabled": False}, headers=H1).status_code == 404
...
def test_only_admins_may_manage_rules(tmp_path):
    with _client(tmp_path) as client:
        ...
        assert client.post("/automation/rules", json=_valid_rule(), headers=H2).status_code == 403
...
def test_rule_created_over_http_fires_in_the_engine(tmp_path):
    ...
    with TestClient(create_app(db_file)) as client:
        ...
        assert client.post("/automation/rules", json=body, headers=H1).status_code == 201
