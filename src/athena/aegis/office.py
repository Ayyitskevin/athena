...
from athena.aegis import issues, leases
from athena.core import fleet_roster, users
...
def _chair_from_lease(
    conn: sqlite3.Connection, lease: dict, seat_slug: str | None
) -> dict:
...
def build_office(
    ...
    *,
    ...
    chair = None
    if len(active) == 1:
        chair = _chair_from_lease(conn, active[0], seat_slug)
    elif len(active) > 1:
        warnings.append(
